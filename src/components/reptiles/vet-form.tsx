...
export function VetForm({ reptileId, visit, onSuccess, onCancel }: VetFormProps) {
  ...
  const validate = () => {
    ...
    if (formData.reason.length > 500) newErrors.reason = 'Reason must be 500 characters or less'
    if (formData.cost && isNaN(parseFloat(formData.cost))) newErrors.cost = 'Cost must be a number'
    if (formData.cost && parseFloat(formData.cost) < 0) newErrors.cost = 'Cost must be non-negative'
    setErrors(newErrors)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label htmlFor="cost" className="block text-sm font-medium text-warm-700 mb-1">
            Cost
          </label>
          <Input
            id="cost"
      ...
      <div>
        <label htmlFor="reason" className="block text-sm font-medium text-warm-700 mb-1">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label htmlFor="vetName" className="block text-sm font-medium text-warm-700 mb-1">
            Veterinarian Name
          </label>
          <Input
            id="vetName"
        ...
        <div>
          <label htmlFor="vetClinic" className="block text-sm font-medium text-warm-700 mb-1">
            Clinic
          </label>
          <Input
            id="vetClinic"
      ...
      <div>
        <label htmlFor="diagnosis" className="block text-sm font-medium text-warm-700 mb-1">
          Diagnosis
        </label>
        <textarea
          id="diagnosis"
          ...
          onChange={handleChange}
          className="w-full rounded-md border border-warm-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-warm-500"
        />
      </div>

      <div>
        <label htmlFor="treatment" className="block text-sm font-medium text-warm-700 mb-1">
          Treatment
        </label>
        <textarea
          id="treatment"
      ...
      <div>
        <label htmlFor="followUp" className="block text-sm font-medium text-warm-700 mb-1">
          Follow-up Date
        </label>
        <Input
          id="followUp"
