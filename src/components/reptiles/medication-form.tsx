...
export function MedicationForm({ reptileId, medication, onSuccess, onCancel }: MedicationFormProps) {
  ...
  const validate = () => {
    ...
    if (formData.dosage.length > 100) newErrors.dosage = 'Dosage must be 100 characters or less'
    if (!formData.frequency.trim()) newErrors.frequency = 'Frequency is required'
    if (formData.frequency.length > 100) newErrors.frequency = 'Frequency must be 100 characters or less'
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label htmlFor="dosage" className="block text-sm font-medium text-warm-700 mb-1">
        ...
        <div>
          <label htmlFor="frequency" className="block text-sm font-medium text-warm-700 mb-1">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          onChange={handleChange}
          className="h-4 w-4 rounded border-warm-300 text-blue-600 focus:ring-blue-500"
        />
        <label htmlFor="reminders" className="text-sm text-warm-700">
